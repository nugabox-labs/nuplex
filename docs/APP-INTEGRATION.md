...
앱 셸이 넣은 것: `PUBLIC_PATHS` 에 `/api/app/config` (셸은 로그인 전에 호출한다).
...
| `POST /api/app/push/token` | 푸시 토큰 등록 · 갱신 | 열람 세션 |
| `DELETE /api/app/push/token` | 로그아웃 · 알림 끄기 | 열람 세션 |

`/api/app/config` 에 인증을 걸지 않은 이유: 셸이 **로그인 전에** 부르고, 담긴 값도
...
웹을 배포하면 앱도 같이 바뀐다. 앱 셸에서 주요 흐름(로그인 → 프로필 선택 → 홈 →
